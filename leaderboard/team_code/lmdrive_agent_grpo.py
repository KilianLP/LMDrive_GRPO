...
class LMDriveAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        model_cls = registry.get_model_class('vicuna_drive_grpo')
    ...
    def run_step(self, input_data, timestamp):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            waypoints, is_end = self.net(input_data, inference_mode=False, image_embeds=image_embeds)

        waypoints = waypoints[-1]
        waypoints = waypoints.view(5, 2)
        std = torch.ones_like(waypoints) * self.config.std
        dist = torch.distributions.Normal(loc=waypoints, scale=std)
        sample_waypoints = dist.sample()
        end_prob = self.softmax(is_end)[-1][1] # is_end[1] means the prob of the frame is the last frame

        steer, throttle, brake, metadata = self.control_pid(sample_waypoints, velocity)
        ...
        return control, sample_waypoints, waypoints, input_data, end, end_prob
    ...
    def train(self, data):
        ...
        for _ in range(update_iters):
            ...
            means = self.net(data["obs"])
            dist = torch.distributions.Normal(loc=means, scale=0.1*torch.ones_like(means))
