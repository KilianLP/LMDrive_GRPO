...
from leaderboard.scenarios.scenario_manager_grpo import ScenarioManager
...
class LeaderboardEvaluator(object):
    ...
    def _register_statistics(self, config, checkpoint, entry_status, crash_message=""):
        ...
        score = current_stats_record.scores['score_composed']
    ...
    def _load_and_run_scenario(self, args, config, i_try):
        """
        ...
        try:
            ...
            score = self._register_statistics(config, args.checkpoint, entry_status, crash_message)
        ...
        while self.manager._running:
            ...
            if timestamp:
                waypoints, sample_waypoints, input_data, is_end, end_prob = self.manager._tick_scenario(timestamp)
                if self.manager._running == True:
                    self.grpo_buffer.store(input_data, sample_waypoints, 0, waypoints, is_end, end_prob, False, i_try)
        ...
        try:
            ...
            score = self._register_statistics(config, args.checkpoint, entry_status, crash_message)
            self.grpo_buffer.store(input_data, sample_waypoints, score, waypoints, is_end, end_prob, True, i_try)
    ...
    def train(self):
        data = self.grpo_buffer.get()
        self.agent_instance.train(data)
    

    def run(self, args, batch_size=1):
        """
        ...
        while route_indexer.peek():
            ...
            for i in range(self.n_try):
                self._load_and_run_scenario(args, config, i)

            if n_route % self.batch_size == 0:
                self.train()
